...
- [A perfectly normally distributed sample: another post?](https://easystats.github.io/blog/posts/bayestestr_distribution/)
...
Taking _all_ packages from the **easystats** project into account, we have following package dependencies.
...
The [**see**](http://easystats.github.io/see) package is one of our high-level packages that is responsible for plotting and creating figures. Thus, it increases package dependencies by a substantial amount. Just taking the core packages into consideration, you see that the **easystats** packages are indeed small and lightweight.
...
dependencies <- c("insight", "performance", "bayestestR", "parameters", "correlation", "report", "estimate")
...
data <- rbind(downloads_insight,
  ...
  ungroup() %>% 
  mutate(Package = fct_relevel(Package, "insight", "bayestestR", "performance", "see"))
...
blogposts$Package <- rev(c("insight", "bayestestR", "bayestestR", "performance", "bayestestR", "see", "bayestestR", "bayestestR", "performance"))
...
packages_colours <- c("insight" = "#FF9800",
                      "bayestestR" = "#E91E63",
                      "performance" = "#4CAF50",
                      "see" = "#2196F3")
```
...
data %>% 
  ...
  geom_line(aes(y=count), size=0.5, linetype="dotted") +
  geom_smooth(aes(y=count), method = 'loess', size=1.75, se=FALSE) +
  geom_smooth(aes(y=count), method = 'lm', size=0.75, se=FALSE) +
  ...
  geom_text_repel(data=events, aes(label = event, x = date, y = event_y), colour = events$color, segment.colour = events$color, size = 3) +
  see::theme_modern() +
