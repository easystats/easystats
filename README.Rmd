...
ndeps <- lapply(easystats_pkgs[,"Package"], pkgDep, availPkgs = easystats_pkgs,
                depends = TRUE, suggests = FALSE, enhances = FALSE, 
                includeBasePkgs = FALSE)
...
# Packages data
downloads_all <- map_dfr(pkgsnms, cranlogs::cran_downloads, from = "2019-02-26")
...
data <- full_join(data, events, by = c("date", "Package")) %>% 
  mutate(Package = fct_reorder(Package, count, .fun = sum, .desc = TRUE))
