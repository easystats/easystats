...
As we can see, the only exception is the [**see**](http://easystats.github.io/see) package, one of our high-level packages that is responsible for plotting and creating
...
data <- data_join(data, events, by = c("date", "Package")) %>%
  mutate(Package = reorder(Package, count, sum, decreasing = TRUE))
...
names(dl_table) <- dl_table[1, ]
dl_table <- dl_table[-1, ]
dl_table[] <- lapply(dl_table, as.numeric)
dl_table <- dl_table %>% 
  mutate(Total = rowSums(dl_table[1, ])) %>% 
...
data %>%
  ...
  ungroup() %>%
  mutate(Package = relevel(Package, "bayestestR", names(packages_colours))) |>
...
average_monthly_downloads %>%
  mutate(package2 = reorder(package2, monthly_downloads, max)) %>%
