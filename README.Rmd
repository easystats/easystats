...
[**tidyverse**](https://tidyverse.org/), another very popular collection of
...
breaks <- seq(
  from = lubridate::floor_date(date_range[1], "year"),
  to = date_range[2],
  by = "6 months"
)
...
```{r easystats_citations, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE}
easystats::easystats_citations(sort_by = "cites")
```

```{r easystats_citations_hide, warning=FALSE, message=FALSE, echo=FALSE}
cites <- easystats::easystats_citations(sort_by = "cites", length = 50)
if (!is.null(cites)) {
  insight::print_md(cites)
}
```
