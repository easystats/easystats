...
tryCatch(
  ...
  error = function(e) {
    performance_args[[1]] <- NULL
    args <- vector(length = length(performance_args))
    for (i in seq_along(performance_args)) {
      args[i] <- paste(",", names(performance_args)[i], "=", performance_args[[i]])
    } 
    code_to_run <- paste0("table_performance <- performance::performance(", params$model_name, paste(args, collapse = ""), ")\n  print_md(table_performance, layout = \"vertical\", caption = NULL)")
...
tryCatch(
  ...
  error = function(e) {
    parameters_args[[1]] <- NULL
    args <- vector(length = length(parameters_args))
    for (i in seq_along(parameters_args)) {
      args[i] <- paste(",", names(parameters_args)[i], "=", parameters_args[[i]])
    } 
    code_to_run <- paste0("parameters::parameters(", params$model_name, paste(args, collapse = ""), ")")
    
    cat(insight::format_message(
      "\nSomething did not work as expected. Try to run the following code:\n", code_to_run, "\nIf there is an error, please file an issue at {.url https://github.com/easystats/parameters/issues/} with the output of the code above. If you don't have an error in R, please file an issue at {.url https://github.com/easystats/easystats/issues/} and post the following output:\n", e$message
    ))
  }
)
```
...
tryCatch(
  ...
  error = function(e) {
    ...
    code_to_run <- paste0("table_parameters <- parameters::parameters(", params$model_name, paste(args, collapse = ""), ")\n  print_md(table_parameters, caption = NULL)")
...
if (!is.null(text_modelbased)) {
  ...
  code_to_run <- paste(
"text_modelbased <- lapply(unique(c(\"", paste(model_terms, collapse = "\", \""), "\")), function(i) {
    grid <- get_datagrid(", 
      params$model_name, ", 
      at = i,
      range = \"grid\",
    ...
    estimate_expectation(", params$model_name, ", data = grid, verbose = FALSE)
 })
...
sep = ""
      ...
      cat(insight::format_message(
        "\nSomething did not work as expected. Try to run the following code:\n", code_to_run, "\n\nIf there is an error, please file an issue at {.url https://github.com/easystats/easystats/issues/} with the output of the code above.", line_length = 1000
      ))
    
  format_next_chunk <- ifelse(is.null(text_modelbased), "markup", "asis")
}
```
...
```{r easydashboard-9, eval=is_supported, results = if (is.null(text_modelbased)) "markup" else "asis"}
if (!is.null(text_modelbased)) {
