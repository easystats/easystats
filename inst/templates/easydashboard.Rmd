...
# modelbased
model_terms <- find_interactions(model, component = "conditional", flatten = TRUE)
if (is.null(model_terms)) {
  model_terms <- find_variables(model)$conditional
}
text_modelbased <- lapply(model_terms, function(i) {
...
see::plots(lapply(text_modelbased, plot), n_columns = round(sqrt(length(text_modelbased))))
```
