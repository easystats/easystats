...
shorttitle        : "Bayesian Indices of Existence and Significance"
...
author: 
  ...
  - name          : "S.H. Annabel Chen"
    affiliation   : " 1, 3, 4, *, $\\dagger$"
    
  - name          : "Daniel Lüdecke"
    affiliation   : " 5, $\\dagger$"
...
```{r Table1, message=FALSE, warning=FALSE, echo=FALSE, out.width = "\\textwidth", results = "asis"}
kable(table1, "latex", digits = 2, row.names = FALSE, booktabs = TRUE, caption = "Sensitivity to sample size. This table shows the standardized coefficient between the sample size and the value of each index, adjusted for error, and stratified by model type and presence of true effect. The stronger the coefficient is, the stronger the relationship with sample size.") %>% 
...
```{r Table2, message=FALSE, warning=FALSE, echo=FALSE, out.width = "\\textwidth", results = "asis"}
kable(table2, "latex", digits = 2, row.names = FALSE, booktabs = TRUE, caption = "Sensitivity to noise. This table shows the standardized coefficient between noise and the value of each index when the true effect is present, adjusted for sample size and stratified by model type. The stronger the coefficient is, the stronger the relationship with noise.") %>% 
  kableExtra::kable_styling(font_size = 9) %>%
...
```{r Figure4, message=FALSE, warning=FALSE, fig.cap='Relationship with the frequentist *p*-value. In each plot, the *p*-value densities are visualized by the marginal top (absence of true effect) and bottom (presence of true effect) markers, whereas on the left (presence of true effect) and right (absence of true effect), the markers represent the density of the index of interest. Different point shapes, representing different sample sizes, specifically illustrate its impact on the percentages in ROPE, for which each "curve line" is associated with one sample size (the bigger the sample size, the higher the percentage in ROPE).', fig.align='center', fig.width=21/2, fig.height=29.7/2, echo=FALSE, out.width = "0.65\\textwidth", results = "asis", fig.pos = "!ht"}
...
```{r Figure5, message=FALSE, warning=FALSE, fig.cap="The probability of reaching different *p*-value based significance thresholds (.1, .05, .01, .001 for solid, long-dashed, short-dashed and dotted lines, respectively) for different values of the corresponding Bayesian indices.", fig.align='center', fig.width=29.7/2, fig.height=21/2, echo=FALSE, out.width = "0.9\\textwidth", results = "asis", fig.pos = "!ht"}
...
```{r Figure6, message=FALSE, warning=FALSE, fig.cap="Relationship between three Bayesian indices: The Probability of Direction (*pd*), the percentage of the full posterior distribution in the ROPE, and the Bayes factor (*vs.* ROPE).", fig.align='center', fig.width=21/1.8, fig.height=29.7/1.8, echo=FALSE, out.width = "0.9\\textwidth", results = "asis", fig.pos = "!ht"}
...
```{r table3, message=FALSE, warning=FALSE, echo=FALSE, out.width = "\\textwidth", results = "asis"}
kable(table3, "latex", booktabs = TRUE, caption = "Summary of Bayesian Indices of Effect Existence and Significance.") %>% 
  kableExtra::kable_styling(font_size = 6, latex_options = c("hold_position")) %>%
  kableExtra::column_spec(2:5, width = "3cm") 
```
...
Defining appropriate heuristics to help the interpretation is beyond the scope of this paper, as it would require testing them on more natural datasets. Nevertheless, if we take the frequentist framework and the existing literature as a reference point, it seems that 95\%, 97\% and 99\% might be relevant reference points (i.e., easy-to-remember values) for the *pd*. A concise, standardized, reference template sentence to describe the parameter of a model including an index of point-estimate, uncertainty, existence, significance and effect size [@cohen1988statistical] could be, in the case of *pd* and *BF*:

"There is moderate evidence ($BF_{ROPE} = 3.44$) [*BF (vs. ROPE)*] in favor of the presence of effect of X, which has a probability of $98.14\%$ [*pd*] of being negative ($Median = -5.04$, $89\% CI [-8.31., 0.12]$), and can be considered as small ($Std. Median = -0.29$) [*standardized coefficient*]"

And if the user decides to use the percentage in ROPE instead of the *BF*:

"The effect of X has a probability of $98.14\%$ [*pd*] of being negative ($Median = -5.04$, $89\% CI [-8.31, 0.12]$), and can be considered as small ($Std. Median = -0.29$) [*standardized coefficient*] and significant ($0.82\%\text{ in }ROPE$) [*ROPE (full)*]".
