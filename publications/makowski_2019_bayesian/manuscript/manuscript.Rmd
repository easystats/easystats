...
```{r Figure1, message=FALSE, warning=FALSE, fig.cap="**Figure 1**. Bayesian indices of effect existence and significance. (A) The Probability of Direction (*pd*) is defined as the proportion of the posterior distribution that is of the median's sign (the size of the yellow area relative to the whole distribution). (B) The MAP-based *p*-value is defined as the ratio of posterior density at 0 (the height of the red lollipop) divided by the density at the Maximum A Posteriori (MAP; the height of the blue lollipop). (C) The percentage in ROPE corresponds to the red area relative to the distribution (with or without tails for ROPE (*full*) and ROPE (*95%*), respectively). (D) The Bayes factors indicates the degree by which the mass of the parameter distribution has shifted away from or closer to the null. When the null is a point, the Bayes factor (*vs. 0*) is the point-null density of the prior distribution (the blue lollipop on the dotted distribution) divided by that of the posterior distribution (the red lollipop on the yellow distribution); When the null is the ROPE, the Bayes factor (*vs. ROPE*) is the odds of the parameter falling inside-vs-outside the ROPE of the prior distribution (the blue area on the dotted distribution) divided by that of the posterior distribution (the red area on the yellow distribution).", fig.align='center', fig.width=29.7/2, fig.height=21/2, echo=FALSE}
