...
MSB: pd is a measure certinty about the effects direction - when it is hugh, we are cirtain that the effect has a direction. But when it is low (~50\%) we cannot say that we are certain the effect has no direction, but only that we are uncirtain about the effects direction. This is why, like the *p*-value, *pd* can and should only be used for accepting the alternative / rejecting the null, but not for accepting the null.
