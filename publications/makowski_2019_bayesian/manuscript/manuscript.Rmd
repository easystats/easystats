...
The **Figure 3** suggests that the *pd* has a 1:1 correspondance with the frequentist *p*-value. *BF* indices still appear as having a strong relationship (altough severly non-linear) with the frequentist index, <!-- {MSB} which is to be expected as the BF is consistant when H0 and when H1 is true, whereas the *p*-value is only consistant (reflects the "truth" as sample size increases) when H1 is true, but has a uniform distribution [0-1] when H0 is true, -->while *ROPE*-based percentages appear as being the most independent.
...
<!-- MSB: This plot seems to me like a reflection of the fact that ROPE and BF are consistant both when H0 and H1 are true, while pd (like p-value) is only consistant when H1 is true... -->
