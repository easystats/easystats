...
<!-- why bayes and underlining issues related to p-value -->
...
Indeed, while one of the strengths of the Bayesian framework is its probabilistic parameter estimation, allowing to quantify the inherent uncertainty associated with each estimation, psychologists are also interested in indices that would allow them to conclude if a given parameter affects substantially the outcome in a given direction. This need has led to the wide adoption of the frequentist *p*-value and its acceptance was accompanied with the creation of arbitrary clusters for its interpretation (*.05*, *.01* and *.001*). Unfortunately, these heuristics have severely rigidified, becoming a goal and threshold to reach rather than a tool for understanding the data [@cohen2016earth; @kirk1996practical]. Thus, the ability of the Bayesian framework to answer psychological questions without the need of such null-hypothesis testing indices is often promoted as the promise of a "a new world without *p*-value", as opposed to the old and flawed frequentist one. 

<!-- Utility of significance -->

Nonetheless, it seems that an index of "significance", as well heuristics to interpret it are useful for Humans to gain an intuitive understanding of the interactions and structure of their data. While desire for significance assessment can be pragmatically observed in the tenacity and resilience of the *p*-value (which, despite a wide agreement on its defects and countless attacks, is still widely used and taught), its undeyling cause could be linked to the practical dissociation with another perspective on effects description, more focused on *estimation*. For instance, in some fields, the goal of a study is to precisely estimate a given effect (the question being for example is a given coefficient closer to 7 or to 9?). On the contrary, in other fields, such as for instance experimental psychology, the implicit aim has often been focused on *significance* (altough wether it is a cause or a consequence of the historical heavy use of ANOVAs is unclear): "are these two conditions different?", "is there a correlation between these two variables?", with little care to the precise estimation of the parameter of interest.

<!-- From significance to decision -->

<!-- From significance to decision (medical science etc.). -->
...
<!--Present Bayesian indices -->

<!-- Bayesian indices could be roughly grouped into 3 overlapping categories: posterior indices, ROPE-based indices and Bayes factors. -->
...
**Figure 3** suggests that the *pd* has a 1:1 correspondance with the frequentist *p*-value. *BF* indices still appear as having a strong relationship (altough severly non-linear) with the frequentist index, which is to be expected since the *BF* is consistant (i.e., reflects the "truth" as sample size increases) both when the null is true and when the alternative is ture (as can be seen in the marginal distibutions, marked by colored dashes in the plot margins), whereas the *p*-value is only consistant when the alternative is true, but has a uniform distribution [0-1] when the null is true. *ROPE*-based percentages appear to be only weakly related to *p*-values (***REF***). <!-- MSB: somewhere I know there is a paper the dicusses this - I will hunt it down! -->
