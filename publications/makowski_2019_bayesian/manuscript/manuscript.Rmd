...
```{r CommonAesthetics, message=FALSE, warning=FALSE, fig.cap="Figure 2.", fig.align='center', fig.width=21/2, fig.height=29.7/2, dpi=300, echo=FALSE}
# Common Aesthetics
indices <- c("p_value" = "p-value",
...
```{r Figure1, message=FALSE, warning=FALSE, fig.cap="Figure 1. Impact of Sample Size.", fig.align='center', fig.width=21/2, fig.height=29.7/2, dpi=300, echo=FALSE}
figure1_data <- df %>%
  ...
  filter((index == "BF_log" & value <= log(999)) | index != "BF_log",
...
```{r Figure2, message=FALSE, warning=FALSE, fig.cap="Figure 2. Impact of Noise.", fig.align='center', fig.width=21/2, fig.height=29.7/2, dpi=300, echo=FALSE}
figure2_data <- df %>%
...
```{r PerformanceComparison, message=FALSE, warning=FALSE}
library(parameters)
library(performance)
...
model_BF_ROPE_log <- df %>%
  parameters::normalize(select="BF_ROPE_log") %>%
  filter(outcome_type == model_type) %>% 
  glm(true_effect ~ BF_ROPE_log + error + sample_size, data=., family="binomial")

  performance_table <- performance::compare_performance(model_p_value, model_p_direction, model_p_MAP, model_ROPE_95, model_ROPE_full, model_BF_log, model_BF_ROPE_log)
...
```{r Figure3, message=FALSE, warning=FALSE, fig.cap="Figure 3. Relationship with the frequentist p-value.", fig.align='center', fig.width=21/2, fig.height=29.7/2, dpi=300, echo=FALSE}
figure3_data <- df %>%
  select(outcome_type, true_effect, error, sample_size, p_value, p_direction, p_MAP, ROPE_95, ROPE_full, BF_log, BF_ROPE_log) %>%
...
```{r Figure4, message=FALSE, warning=FALSE, fig.cap="Figure 4. Relationship between Bayesian indices and frequentist arbitrary threshold of significance.", fig.align='center', fig.width=21/2, fig.height=21/2, dpi=300, echo=FALSE}
...
figure4_data <- df %>% 
  ...
  mutate(threshold = factor(threshold, levels = c("sig_1","sig_05","sig_01","sig_001")),
         index = factor(index, levels = c("p_direction", "p_MAP", "ROPE_95", "ROPE_full", "BF_log", "BF_ROPE_log"))) %>% 
  filter((index == "p_direction" & value >= 0.925) | index != "p_direction",
...
figure4 <- ggplot(figure4_data, aes(x=value, y=sig, linetype = threshold, group = interaction(outcome_type, threshold))) +
  ...
  scale_linetype_manual(values = c("sig_1" = "dotted", "sig_05" = "dashed", "sig_01" = "longdash", "sig_001" = "solid"),
...
```{r Figure5, message=FALSE, warning=FALSE, fig.cap="Figure 5. Relationship between three Bayesian indices.", fig.align='center', fig.width=21/1.75, fig.height=29.7/1.75, dpi=300, echo=FALSE}
pd_rope <- df %>%
  mutate(true_effect = as.factor(true_effect),
