...
- **Figure 1**. Bayesian indices of effect existence and significance. (A) The Probability of Direction (*pd*) is defined as the proportion of the posterior distribution that is of the median's sign (the yellow area). (B) The MAP-based *p*-value is defined as the density value at 0, - the height of the red segment, divided by the density at the Maximum A Posteriori (MAP), - the height of the blue segment. (C) The ROPE (*95%*) is defined as the proportion of the 95% CI (the yellow area) that lies within the ROPE (the red area). For the ROPE (*full*), the full posterior distrubtion (including the tails) is used. (D) Bayes factors indicate the degree by which the posterior distribution has shifted from the null value (0) or the ROPE (the red area), relative to the prior distribution (represented by the dotted line).
